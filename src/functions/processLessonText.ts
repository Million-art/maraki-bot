...
import { GrammarEntry, LessonEntry } from "../types/conversation.js";
...
// Main function
async function processLessonText(ctx: any, userText: string, userId: number) {
  ...
  try {
    ...
    let conversation = await Conversation.findOne({ userId }) as ConversationDoc;
    if (!conversation) {
      conversation = await Conversation.create({ userId }) as ConversationDoc;
    }
...
${conversation.lesson
  ...
  .join("\n")}
Current context: The user is learning about "${
          conversation.currentLesson || "General Lesson"
        }".
