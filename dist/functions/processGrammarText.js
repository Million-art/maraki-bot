 ...
 */
async function processGrammarText(ctx, userText, userId) {
    ...
    try {
        ...
        if (conversation.grammar.length > 0) {
            const namePatterns = conversation.grammar
                .filter((conv) => conv.input.toLowerCase().includes("my name is") ||
            ...
            if (namePatterns.length > 0) {
                ...
                if (nameMatch)
                    userInfo = `User's name: ${nameMatch[1]}. `;
            }
        }
...
User text: "${userText}"`;
        ...
        try {
            ...
            await ctx.reply(formatResponse("❌ Invalid AI response. Please try again.", "error"));
            return;
        }
